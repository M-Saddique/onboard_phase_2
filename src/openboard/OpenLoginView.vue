...
<!--          </v-col>-->
          <v-col>
            <v-card-text style="color: white; padding: 0px; text-align: center">
              Don't have an account? <span class="signUpLink" onclick="window.parent.location.href= '/order/AccountRecovery'">Retrieve Now!</span>
            </v-card-text>
          </v-col>
        ...
        axios.post(url, data, {
          ...
          if(response.status == 200){
            ...
            if(processLogin['Failure']){
              ...
              this.$router.push('order/openOrders')
